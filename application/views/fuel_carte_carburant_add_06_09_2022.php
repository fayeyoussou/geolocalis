                  ...
                  <div class="row">
                   <input type="hidden" name="cc_id" id="cc_id" value="<?php echo (isset($fuel_carte_carburantdetails)) ? $fuel_carte_carburantdetails[0]['cc_id']:'' ?>" >
					...
					<div class="col-sm-4 col-md-4">
                          <label class="form-label">Compagnie petrolière<span class="form-required">*</span></label>
                      <div class="form-group">
                         <select id="cc_compagnie_id"  class="form-control selectized"  name="cc_compagnie_id" required="true">
                        <option value="">Selectionner la Compagnie petrolière</option>
                        <?php  foreach ($carte_carburantcompagnielist as $key => $carte_carburantcompagnielists) { ?>
                        <option <?php if((isset($fueldetails)) && $fueldetails[0]['cc_compagnie_id'] == $carte_carburantcompagnielists['ccc_id']){ echo 'selected';} ?> value="<?php echo output($carte_carburantcompagnielists['ccc_id']) ?>"><?php echo output($carte_carburantcompagnielists['ccc_name']); ?></option>
                        <?php  } ?>
                     </select>
                      </div>
                    </div>					  
