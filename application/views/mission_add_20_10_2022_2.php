...
$mi_annulation=''; $mi_annulation =  $missions['mi_annulation']; 							   
...
<a class="btn btn-xs btn-default"  onclick="click_here(this.id, this.name)"  id= " {{$entry->getKey()}} "  target="123" name = "approve" class="approve_btn"> <i class="fa fa-check"> </i> Approve </a>
<a class="btn btn-xs btn-default"  onclick="click_here(this.id, this.name)"  id= " {{$entry->getKey()}} "  name = "disapprove" class="approve_btn"> <i class="fa fa-times"> </i> Disapprove </a>

</div>								  
<!--FIN-->								  
								  
					  <!----><div class="mt-2 mb-3">
					    <a href="#" id="<?php echo output($missions['mi_id']); ?>" class="btn btn-sm btn-danger <?//= ($incomexpensedetails['ie_amount'] - $totalpaidamt==0)?'disabled':'' ?>" data-toggle="modal" data-target="#modal-pregate">Ajouter pregate</a><br>
					  </div>                                      
						<i class="fa fa-ban text-danger"></i>dfsd		  
                     <?php }  // fin controle?>  
                     <?php }  // fin controle?>  
...
<script>
   ...
   {
            var num  = clicked_id;
            var name = clicked_name;
            $.ajax({
                url:'/getMsg/{num1}',
                type:'POST',
                data: { "num1": num,  _token: '{{csrf_token()}}', "name": name},
                success:function($data){
