    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-8">
            <h1 class="m-0 text-dark"><?php //echo (isset($incomexpensedetails))?'':'' ?> <!--N° B--><?//= $numero_compteurbanque; ?>
            
<?php if((isset($incomexpensedetails))){ ?>Modifier une opération de banque N° <?php echo (isset($incomexpensedetails)) ? $incomexpensedetails[0]['ie_numero']:'' ?>  <?php }else{?> Ajouter une opération de banque N° B<?= $numero_compteurbanque; }?>			  
...
<section class="content">
      <div class="container-fluid">
       	     <form method="post" id="Income_Expense" class="card" action="<?php echo base_url();?>Incomexpense/<?php echo (isset($incomexpensedetails))?'updateincomexpense_banque':'insertincomexpense_banque_entree'; ?>" enctype="multipart/form-data">
          <div class="card-body">
	...
	<?php if((isset($incomexpensedetails)) && $incomexpensedetails[0]['ie_recu_bordereau_versement'] != '') {?>
                 ...
                 <input type="hidden" id="ie_created_date" name="ie_created_date" value="<?php echo date('Y-m-d h:i:s'); ?>">
                  ...
                  <input type="hidden" id="ie_espece_cheque" name="ie_espece_cheque" value="<?php echo "CHEQUE"; ?>"><!--ENTREE-->
			        <?php if((isset($incomexpensedetails))){ }else{?><input type="hidden" id="ie_numero_enregistrement" name="ie_numero_enregistrement" value="B<?= $numero_compteurbanque; ?>"><?php }?>
...
<section class="content">
      <div class="container-fluid">
       	     <form method="post" id="Income_Expense" class="card" action="<?php echo base_url();?>Incomexpense/<?php echo (isset($incomexpensedetails))?'updateincomexpense_banque':'insertincomexpense_banque_sortie'; ?>">
          <div class="card-body">
...
<!--<div id="ie_transitaire_id" name="ie_transitaire_id">
         ...
         <div class="card-body">
            <div class="row">
				...
				<div class="col-sm-6 col-md-6">
                  <div class="form-group">
                     <label class="form-label">Compte<span class="form-required">*</span></label>
                     <select   class="form-control selectized"  name="ie_compte_id" >
                        <option value="">Sélectionner le Compte</option>
                        <?php  foreach ($comptelist as $key => $comptelists) { ?>
                        <option <?php if((isset($incomexpensedetails)) && $incomexpensedetails[0]['ie_compte_id'] == $comptelists['iec_id']){ echo 'selected';} ?> value="<?php echo output($comptelists['iec_id']) ?>"><?php echo output($comptelists['iec_name']); ?> - Code: <?php echo output($comptelists['iec_code']); ?></option>
                        <?php  } ?>
...
<?php  //$t_validation =  $facturelists['t_validation'];?>                           
                            ...
                            <a class="icon" href="<?php echo base_url(); ?>incomexpense/editincomexpense_banque/<?php echo output($incomexpenses['ie_id']); ?>">
...
$(document).ready(function(){
 $('#ie_compte_id').change(function(){
  var ie_compte_id = $('#ie_compte_id').val();
  if(ie_compte_id != '')
  {
   $.ajax({
     url:"<?php echo base_url(); ?>incomexpense/fetch_transitaire",
 //  url:"<?php echo base_url(); ?>mission/fetch_state",
    method:"POST",
    data:{ie_compte_id:ie_compte_id},
    success:function(data)
    {
     $('#ie_transitaire_id').html(data);
...
$('#mi_vehicle_id').change(function(){
  ...
  {
   $.ajax({
     url:"<?php echo base_url(); ?>incomexpense/fetch_bon_carburant",
  // url:"<?php //echo base_url(); ?>incomexpense/fetch_transitaire",
    method:"POST",
