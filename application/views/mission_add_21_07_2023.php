...
<!-- Fin Carte Carburant-->                    
  ...
  <!----><div class="col-sm-6 col-md-3">
                  <div class="form-group">
                     <label class="form-label"> Type de livraison<span class="form-required">*</span></label>
				  
<!-- BEGIN SELECT-->
					  
   <select name="mi_type_transport" id="mi_type_transport" class="form-control selectized" required>
    <option value="">Selectionner le Type de livraison</option>
    <?php
...
<?php if(isset($missiondetails)) { foreach($type_transport as $row) { ?>
                          <option <?= (isset($missiondetails[0]['mi_type_transport']) && $missiondetails[0]['mi_type_transport'] == $row['ta_id'])?'selected':''?> value="<?= $row['ta_id'] ?>"><?= $row['ta_name'] ?></option> 
                          <?php } } else {
