...
/*(isset($$compagniedetails['cc_name']))?$compagniedetails['cc_name']:'Non renseigné';*/
	 
if(isset($compagniedetails['cc_name']))
{
...
$montant_peage = 0;    
$montant_peage = $facturedetails['t_peage_amount'];   

    if($montant_peage>0)
{
        $montant_peageAfficher =0;
		$montantTTCArrondi=$montantTTCArrondi+$montant_peage;
        $montant_peageAfficher = number_format($montant_peage, 0, ',', ' ');
//        $rabaisAfficher = number_format($rabais, 0, ',', ' ');
}	
	

$ristourne_additional = 0;    
$ristourne_additional = $facturedetails['t_ristourne_additional'];   	
	
    if($ristourne_additional>0)
{
        $ristourne_additionalAfficher =0;
		$montantTTCArrondi=$montantTTCArrondi-$ristourne_additional;
        $ristourne_additionalAfficher = number_format($ristourne_additional, 0, ',', ' ');
//        $rabaisAfficher = number_format($rabais, 0, ',', ' ');
}		
// End cool printing	
...
$html .= '<h5><tr style="background-color:#F2F2F2;color:#000000;">
            ...
            <td></td>
            <td></td>
            <td>Ristourne additional</td>
            <td>'.$ristourne_additionalAfficher.'</td>
            </tr></h5>';
}	
...
/**/  if($conteneurdetails['zone_name']->z_montant_conteneur_20!='') {
 ...
 $montant_conteneur = $conteneurdetails['co_montant'];//exit;
 $montant_conteneurAffiche = number_format($montant_conteneur, 0, ',', ' '); 
  // Fin calcul du montant du type de contenteur
...
/*    if(($facturedetails['t_type_facturation'])=="Exportation")*/
	 // Debut du controle if
if($facturedetails['t_taxe']=='avec_tva')
	 {
     $ags=1271;   
