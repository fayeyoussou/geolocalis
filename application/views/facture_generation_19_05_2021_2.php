                       ...
                       /**/ if($conteneurdetails['co_type_conteneur']!='') {
                          ...
                          { $montant_conteneur = $conteneurdetails['zone_name']->z_montant_conteneur_20; 
                            ...
                            $montant_conteneur = $conteneurdetails['zone_name']->z_montant_conteneur_40;
                              $type='40 pieds'; 
                          }} //$montant_conteneur=floor($montant_conteneur); 
                             $montant_conteneur = number_format($montant_conteneur, 0, ',', ' ');
              ?> 
        ...
        <div class="table-responsive">
          <table class="table">
            <tr>
              ...
              <td><?//$ags=1250; ?><?= $montantags=number_format($facturedetails['t_nombre_conteneur'] * $ags, 0, ',', ' '); ?>  <?//=$montantags = number_format($montantags, 0, ',', ' '); ?> <?//= output($data['s_price_prefix']).$facturedetails['t_trip_amount'] ags_montant?></td>
            </tr>
            ...
            <tr>
              ...
              <td><?= $montantHT = number_format($montanttotal+$montantags, 0, ',', ' '); //$facturedetails['t_trip_amount'] ?>  <?//=  $montantHT = number_format($montantHT, 0, ',', ' '); ?> <?//= output($data['s_price_prefix']).$facturedetails['t_trip_amount'] ?></td>
            </tr> 
              
<tr>
              <th style="width:50%">Taxe 18%:</th>
              <td><?= $montantTVA=number_format(($montantHT*18)/100, 0, ',', ' '); //$facturedetails['t_trip_amount'] ?>  <?//=  $montantTVA = number_format($montantTVA, 0, ',', ' '); ?>
            ...
            <tr>
              <th>Montant TTC:</th>
              <td><? //$montantTTC= ?><?=  $montantTTC = number_format($montantTVA+$montantHT, 0, ',', ' '); ?></td>
            </tr>
<?php
/*
