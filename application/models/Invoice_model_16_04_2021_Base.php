	...
	public function getall_invoice($trackingcode=false) { 
		...
		if(!empty($invoicedata)) {
			foreach ($invoicedata as $key => $invoicedataval) {
				...
				$newinvoicedata[$key]['i_vechicle_details'] =  $this->db->select('*')->from('zone')->where('z_id',$invoicedataval['i_zone'])->get()->row();//modifier par zone
				$newinvoicedata[$key]['i_driver_details'] =   $this->db->select('*')->from('drivers')->where('d_id',$invoicedataval['i_driver'])->get()->row();
			}
