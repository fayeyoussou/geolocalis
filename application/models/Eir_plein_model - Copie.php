...
class eir_plein_model extends CI_Model{
	...
	public function add_eir_plein($data) { 
		...
		$this->db->insert('trip_mission_eir_plein',$data);
...
WHERE
mi_id=$ei_mission_id AND t_id=mi_trip_id AND co_id=mi_conteneur_id 
AND vr_id=mi_vehicle_remorque_id 
AND t_compagnie=cc_id AND cc_id=clr_compagnie_id
 AND d_id=mi_driver_id GROUP BY mi_id";	 */
	 
	  
	 	/* $where = "mi_id=$ei_mission_id AND t_id=mi_trip_id AND co_id=mi_conteneur_id 
AND vr_id=mi_vehicle_remorque_id 
AND t_compagnie=cc_id AND cc_id=clr_compagnie_id
