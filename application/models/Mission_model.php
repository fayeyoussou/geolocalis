...
/*DEBUT TOUTES LES MISSIONS VALIDEES*/	
	public function getall_mission_validee() { 
		$requete = 1;		
		$missions = $this->db->select('*')->from('trip_mission')->where('mi_validation',$requete)->order_by('mi_id','desc')->get()->result_array();
		if(!empty($missions)) {
			foreach ($missions as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
				$newtripdata[$key]['pregate'] =  $this->db->select('*')->from('trip_pregate')->where('pr_trip_id',$tripdataval['mi_trip_id'])->get()->row();
...
/*FIN TOUTES LES MISSIONS VALIDEES*/	
...
/*DEBUT PRIME */
    ...
    public function getall_mission_prime_tab_paiement() { 
		
		$requete="";
		$requete="mpp_mission_prime_id=mi_id";
		return $this->db->select('*')->from('trip_mission,trip_mission_prime_payments')->get()->result_array();		
//		return $this->db->select('*')->from('trip_mission,trip_mission_prime_payments')->where($requete)->get()->result_array();		

	} 	
	
	public function add_mission_prime($data) { 
		return  $this->db->insert('trip_mission_prime_payments',$data);
...
return $this->db->insert('trip_mission_prime_payments_compteur', $data_trip_mission_prime_payments_compteur);		
		
	}	
	
    public function getall_mission_prime_paiement() { 
...
/*  $this->db->where('bc_compagnie_id', $mi_trip_id);
  ...
  $this->db->order_by('bc_id', 'ASC');*/
		...
		return $this->db->select('*')->from('trip_mission_prime')->where('mp_id NOT IN (SELECT mpp_mission_prime_id FROM trip_mission_prime_payments)')->order_by('mp_id','desc')->get()->result_array();		
...
/*		$requete="";
	...
	public function getall_mission_prime() { 

		$missions = $this->db->select('*')->from('trip_mission_prime')->order_by('mp_id','desc')->get()->result_array();

		if(!empty($missions)) {
			foreach ($missions as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
				$newtripdata[$key]['mission'] =  $this->db->select('*')->from('trip_mission')->where('mi_id',$tripdataval['mp_mission_id'])->get()->row();
				$newtripdata[$key]['facture'] = $this->db->select('*')->from('trips')->where('t_id',$tripdataval['mp_trip_id'])->get()->row();
				/*$newtripdata[$key]['lieu_livraison'] = $this->db->select('*')->from('trip_mission_lieu_livraison')->where('ll_id',$tripdataval['mi_lieu_livraison'])->get()->row();*/
				$newtripdata[$key]['conducteur'] =   $this->db->select('*')->from('drivers')->where('d_id',$tripdataval['mp_driver_id'])->get()->row();
				$newtripdata[$key]['vehicule'] =   $this->db->select('*')->from('vehicles')->where('v_id',$tripdataval['mp_vehicle_id'])->get()->row();
				$newtripdata[$key]['conteneur'] =   $this->db->select('*')->from('trip_conteneur')->where('co_id',$tripdataval['mp_conteneur_id'])->get()->row();
				/*$newtripdata[$key]['lieu_restitution'] =   $this->db->select('*')->from('compagnie_lieu_restitution')->where('clr_id',$tripdataval['mi_lieu_restitution_id'])->get()->row();*/
...
/*	public function viewmission_suite($mi_id) { 
	...
	public function viewmission_suite($mi_id) { 
		$requete="mi_mission_id=$mi_id OR mi_id=$mi_id";
				
//		$missions = $this->db->select('*')->from('trip_mission')->where('mi_id',$mi_id)->get()->result_array();
		$missions = $this->db->select('*')->from('trip_mission')->where($requete)->get()->result_array();
		if(!empty($missions)) {
			foreach ($missions as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
				$newtripdata[$key]['pregate'] =  $this->db->select('*')->from('trip_pregate')->where('pr_trip_id',$tripdataval['mi_trip_id'])->get()->row();
...
/*BEGIN MISSION SUITE REPORT*/
public function viewmission_suite_report($mi_id) { 
	$requete="";
		$requete="mi_mission_id=$mi_id OR mi_id=$mi_id";
		$missions = $this->db->select('*')->from('trip_mission')->where($requete)->get()->result_array();
		if(!empty($missions)) {
...
/*DEBUT LIEU LIVRAISON*/
	...
	public function mission_delete($mi_id) { 

			$this->db->where('mi_id', $mi_id);
    		$this->db->delete('trip_mission');
    		return true;
	//	}
	} 	
/*FIN LIEU LIVRAISON*/	
...
/*DEBUT REPORT JOURNAL PRIME*/
	
	public function reports_journal_prime($from,$to,$driver) { 
//	public function incomexpense_reports($from,$to,$facture,$emettrice,$receptrice) { 
		$tripdataval = array();
		
		if($driver=='all') {
			$where = array('mp_date_mission>='=>$from,'mp_date_mission<='=>$to);
		} else {
			$where = array('mp_date_mission>='=>$from,'mp_date_mission<='=>$to,'mp_driver_id'=>$driver);
		}
		
		 $missions = $this->db->select('*')->from('trip_mission_prime')->where($where)->order_by('mp_id','desc')->get()->result_array();//exit;
if(!empty($missions)) {
			foreach ($missions as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
				$newtripdata[$key]['mission'] =  $this->db->select('*')->from('trip_mission')->where('mi_id',$tripdataval['mp_mission_id'])->get()->row();
     ...
     public function get_numeroprimepayments() { 
		return $this->db->count_all_results('trip_mission_prime_payments_compteur');
}	
	
/*FIN REPORT JOURNAL PRIME*/		
...
//	public function incomexpense_reports($from,$to,$facture,$emettrice,$receptrice) { 
		...
		if($type_transport=='all') {
			$where = array('mi_date_mission>='=>$from,'mi_date_mission<='=>$to);//exit;
		} else {
			$where = array('mi_date_mission>='=>$from,'mi_date_mission<='=>$to,'mi_type_mission'=>$type_transport);
		}
		
		 $incomexpense = $this->db->select('*')->from('trip_mission')->where($where)->order_by('mi_date_mission','desc')->get()->result_array();//exit;
		if(!empty($incomexpense)) {
			foreach ($incomexpense as $key => $incomexpenses) {
				$newincomexpense[$key] = $incomexpenses;

				$newincomexpense[$key]['facture'] =  $this->db->select('t_num_facture,t_customer_id,t_transitaire,t_zone')->from('trips')->where('t_id',$incomexpenses['mi_trip_id'])->get()->row();/**/

				$newincomexpense[$key]['pregate'] =  $this->db->select('pr_numero')->from('trip_pregate')->where('pr_trip_id',$incomexpenses['mi_trip_id'])->get()->row();/**/// pour afficher les noms des banques receptrices
				
				$newincomexpense[$key]['vehicule'] =  $this->db->select('*')->from('vehicles')->where('v_id',$incomexpenses['mi_vehicle_id'])->get()->row(); 
				
				$newincomexpense[$key]['conducteur'] =  $this->db->select('*')->from('drivers')->where('d_id',$incomexpenses['mi_driver_id'])->get()->row(); 		
				
				$newincomexpense[$key]['conteneur'] =  $this->db->select('*')->from('trip_conteneur')->where('co_trip_id',$incomexpenses['mi_trip_id'])->get()->row(); 

				/*$newincomexpense[$key]['zone'] =  $this->db->select('*')->from('zone')->where('co_trip_id',$incomexpenses['mi_trip_id'])->get()->row();*/ 
				
				
				/*$newincomexpense[$key]['transitaire'] =  $this->db->select('*')->from('trips,transitaire')->where('t_id',$incomexpenses['mi_trip_id'],'tra_id','t_transitaire')->get()->row();*/
				
				//$newincomexpense[$key]['transitaire'] =  $this->db->select('*')->from('transitaire')->where('co_trip_id',$incomexpenses['mi_trip_id'])->get()->row(); 				
				/*
				$newincomexpense[$key]['incomeexpense'] =  $this->db->select('ie_numero_enregistrement')->from('incomeexpense')->where('ie_id',$incomexpenses['tja_ie_id'])->get()->row();*/// pour afficher les noms des banques receptrices
