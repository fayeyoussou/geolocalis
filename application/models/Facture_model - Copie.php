...
class facture_model extends CI_Model{
	...
	//	$this->db->insert('facture',$insertdata);
     ...
     public function getfacture_count() { 
		return $this->db->select('*')->from('trips_compteur')->get()->result_array();
	}
		...
		$conteneur =  $this->db->select('*')->from('trip_conteneur')->where('co_trip_id',$t_id)->get()->result_array();
 		if(!empty($conteneur)) {
			foreach ($conteneur as $key => $conteneurs) {
				...
				//$newconteneur[$key]['ags'] =  $this->db->select('*')->from('ags')->get()->row();                   
/*				$newconteneur[$key] = $conteneurs;
				$newconteneur[$key]['z_destination'] =  $this->db->select('z_destination,z_distance,z_montant_conteneur_20,z_montant_conteneur_40')->from('zone')->where('z_id',$conteneurs['co_zone'])->get()->row();
				$newconteneur[$key]['na_name'] =  $this->db->select('na_name,na_montant')->from('trip_nature')->where('na_id',$conteneurs['co_nature'])->get()->row();*/
			}
			return $newconteneur;
		} else 
		{
