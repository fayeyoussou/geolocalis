...
return $this->db->insert('trip_compteur_bon_carburant', $dataCompteur_bon_carburant);		
     ...
     public function get_numerocarburantpayments() { 
		return $this->db->count_all_results('fuel_carte_carburant_recharge_payments_compteur');
}		
...
/**/
   ...
   public function getall_fuel_bon_carburant_list_manager() { 
...
/*		$where = "";
	...
	public function fuel_bon_carburant_paiement_pdf($paiement) { 
		//ccrp_compagnie_id
		return $this->db->select('*')->from('fuel_carte_carburant_recharge_payments,fuel_carte_carburant_compagnie')->where('ccrp_id',$paiement)->get()->result_array();
	}
	
	public function fuel_bon_carburant_paiement__pdf($paiement) { 
	...
	public function updatefuel_bon_carburant() { 
		...
		return $this->db->update('fuel_bon_carburant',$maj_bon_carburant);
	}
...
/*DEBUT BON CARBURANT JOURNAL LISTE*/
	public function getall_fuel_bon_carburant_journal_list() { 
		$newincomexpense = array();
		
			/*$where = array('bc_vehicule_id'=>$bc_vehicule_id);

		$fuel_bon_carburant = $this->db->select('*')->from('fuel_journal_carburant')->where($where)->order_by('fjc_id','desc')->get()->result_array();*/

		$fuel_bon_carburant = $this->db->select('*')->from('fuel_journal_carburant')->order_by('fjc_id','desc')->get()->result_array();
		
		
		if(!empty($fuel_bon_carburant)) {
			foreach ($fuel_bon_carburant as $key => $fuel_bon_carburants) {
				$newfuel_bon_carburant[$key] = $fuel_bon_carburants;
				$newfuel_bon_carburant[$key]['bon_carburant'] =  $this->db->select('bc_numero,bc_quantite,bc_montant,bc_type')->from('fuel_bon_carburant')->where('bc_id',$fuel_bon_carburants['fjc_bc_id'])->get()->row();
	...
	public function getall_fuel_bon_carburant_journal_list_reports($from,$to,$vehicle){ 
		//fuel_reports($from,$to,$v_id) { 
		$newincomexpense = array();
		if($vehicle=='all') {
			$where = array('bc_date >='=>$from,'bc_date<='=>$to);
		...
		$fuel = $this->db->select('*')->from('fuel_journal_carburant')->where($where)->order_by('bc_id','desc')->get()->result_array();
		if(!empty($fuel)) {
			foreach ($fuel as $key => $fuels) {
				$newfuel[$key] = $fuels;
				$newfuel[$key]['vech_name'] =  $this->db->select('v_registration_no,v_name,v_quantite_restant')->from('vehicles')->where('v_id',$fuels['bc_vehicule_id'])->get()->row();
...
/*DEBUT BON CARBURANT JOURNAL*/
	public function fuel_bon_carburant_journal($from,$to,$bc_vehicule_id) { 
		$newincomexpense = array();
		if($v_id=='all') {
			$where = array('bc_date >='=>$from,'bc_date<='=>$to);
		} else {
...
/*END DEMANDE PAIEMENT*/	
	...
	public function fuel_bon_carburant_delete($bc_id) { 

			$this->db->where('bc_id', $bc_id);
    		$this->db->delete('fuel_bon_carburant');
    		return true;
