...
/*	public function getall_facturelist() { 
 ...
 /* Fin carte péage*/    
    ...
    public function getall_mission() { 
		...
		//return $this->db->select('mi_id,v_name,mi_date_mission')->from('trip_mission')->order_by('mi_id','desc')->get()->result_array();
		//t_compagnie=cc_id AND clr_compagnie_id=t_compagnie
		$requete="";
		...
		return $this->db->select('mi_id,mi_numero,v_registration_no,mi_date_mission,pr_numero,clr_name,co_numero_conteneur,d_name,mi_annulation')->from('trip_mission,trip_pregate,vehicles,trip_conteneur,trips,compagnie_lieu_restitution,drivers')->where($requete)->order_by('mi_id','desc')->group_by('mi_id')->get()->result_array();	
...
$output = '<h3> La quantité de carburant restante est égale à: '.$carburant_restant.'</h3>';
