...
//https://www.developpez.net/forums/d626744/php/bibliotheques-frameworks/zend/zend-framework/zend_db/ecrire-jointure-entre-plusieurs-tables-modele-mvc-zend-php5/				
...
/*      $this->db->select("*");
  ...
  $newconteneur[$key]['facture_name'] = $this->db->select("*");
	  $this->db->from('trips');
	  $this->db->join('vehicle_group', 'vehicle_group.gr_id=vehicles.v_group','LEFT');
	  $this->db->order_by('v_id','desc');
	  $query = $this->db->get();
				
/*				$newconteneur[$key]['facture_name'] =  $this->db->select('t_id,t_trip_amount,c_name,tra_name,t_num_facture')->from('trips,customers,transitaire')->where('t_id',$conteneurs['tp_trip_id'])->get()->row(); 
*///				$newconteneur[$key]['facture_name'] =  $this->db->select('*')->from('trips,customers,transitaire')->where('t_id',$conteneurs['tp_trip_id'])->get()->row(); 
				
				//$newfacturedata[$key]['t_customer_details'] =  $this->db->select('*')->from('customers')->where('c_id',$newconteneur['facture_name']->t_customer_id)->get()->row();
