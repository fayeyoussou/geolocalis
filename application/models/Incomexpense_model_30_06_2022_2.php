...
$areceptrice='';
		...
		$reglement='REGLEMENT';
	$where = "";	
	$where = "ie_type_mouvement='$reglement' AND ie_id=tp_ie_id AND ie_trip_id=t_id ";
...
$incomexpense = $this->db->select('*')->from('incomeexpense,trip_payments')->where($where)->order_by('ie_id','desc')->get()->result_array();//exit;*/
		...
		echo $where;//exit;
			...
			$incomexpense = $this->db->select('ie_trip_id,ie_banque_emettrice_id,c_name,ie_banque_receptrice_id,t_num_facture,t_customer_id')->from('incomeexpense,trip_payments,trips')->where($where)->order_by('ie_id','desc')->get()->result_array();//exit;		
