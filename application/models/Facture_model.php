...
/*debut ajout*/
				...
				$newfacturedata[$key]['t_statut_details'] =   $this->db->select('*')->from('trip_statut')->where('st_id',$facturedataval['t_statut_id'])->get()->row();				
/* Fin ajout*/                
...
/*DEBUT FACTURE A VALIDER*/
	public function getall_facture_a_valider() { 
		$newfacturedata = array();
		/*if($trackingcode) {
			$facturedata = $this->db->select('*')->from('trips')->where('t_trackingcode',$trackingcode)->order_by('t_id','desc')->get()->result_array();
		} else {*/
			$statut = "";
			$statut = "non_valide";
			$facturedata = $this->db->select('*')->from('trips')->where('t_validation',$statut)->order_by('t_id','desc')->get()->result_array();
		/*}*/
		if(!empty($facturedata)) {
			foreach ($facturedata as $key => $facturedataval) {
				$newfacturedata[$key] = $facturedataval;
...
/*FIN FACTURE A VALIDER*/	

	
	
	

/*DEBUT FACTURE A ANNULER*/
	public function getall_facture_a_annuler() { 
		$newfacturedata = array();
		/*if($trackingcode) {
			$facturedata = $this->db->select('*')->from('trips')->where('t_trackingcode',$trackingcode)->order_by('t_id','desc')->get()->result_array();
		} else {*/
			...
			$facturedata = $this->db->select('*')->from('trips')->where('t_annulation',$annulation)->order_by('t_id','desc')->get()->result_array();
		/*}*/
		if(!empty($facturedata)) {
			foreach ($facturedata as $key => $facturedataval) {
				$newfacturedata[$key] = $facturedataval;
...
/*BEGIN PENDING INVOICE*/	
	...
	public function getall_facture_instance_reports($from,$to,$statut) { 
		...
		if($statut=='all') {
			...
			$where = "t_date_facture BETWEEN '$from' AND '$to' AND t_validation='$validation' AND t_statut_id=$statut";//exit;
		} else {
			//$where = array('v_fuelfilldate >='=>$from,'v_fuelfilldate<='=>$to,'v_id'=>$statut);
			$where = "t_date_facture BETWEEN '$from' AND '$to' AND t_statut_id='$statut' AND t_validation='$validation' AND t_statut_id=$statut";//exit;
		}
		//$fuel = $this->db->select('*')->from('fuel')->where($where)->order_by('v_fuel_id','desc')->get()->result_array();		
		
		

		  $facturedata = $this->db->select('*')->from('trips')->where($where)->order_by('t_id','desc')->group_by('t_id')->get()->result_array();	
/*		} else {
			//$facturedata = $this->db->select('*')->from('trips')->order_by('t_id','desc')->get()->result_array();
...
/* Fin ajout*/                
	...
	public function getall_facture_instance($trackingcode=false) { 
	    ...
	    $statut = 1;
		//  $facturedata = $this->db->select('*')->from('trip_pregate,trips')->where($where)->order_by('t_id','desc')->group_by('t_id')->get()->result_array();	
//		$where = "t_validation='$validation' AND t_statut_id=$statut";
		$where = "t_statut_id=$statut";
		...
		//	$where = "t_id NOT IN ( SELECT pr_trip_id FROM trip_pregate )";//exit;
			//$facturedata = $this->db->select('*')->from('trips')->order_by('t_id','desc')->group_by('t_id')->get()->result_array();
		$pregate = "";
		$validation = "";
		$validation = "valide";
	    $statut = 1;
...
//		$where = "t_validation='$validation' AND t_statut_id=$statut";
		...
		if(!empty($facturedata)) {
			foreach ($facturedata as $key => $facturedataval) {
				...
				$newfacturedata[$key]['t_pregate_details'] =  $this->db->select('*')->from('trip_pregate')->where('pr_trip_id',$facturedataval['t_id'])->get()->row();
...
/* Fin ajout*/
 ...
 /*BEGIN STATUT*/
	...
	public function get_facturestatut() { 
		return $this->db->select('*')->from('trip_statut')->get()->result_array();
	}

	
	public function facturestatut_delete($gr_id) { 
		$groupinfo = $this->db->select('*')->from('trips')->where('t_statut_id',$st_id)->get()->result_array();
		if(count($groupinfo)>0) {
			return false;
		} else {
...
/*END STATUT*/	
        ...
        public function maj_statut_facture($data) {   
...
/*DEBUT*/	
	public function journal_management($from,$to,$facture) { 
		$newtripdata = array();
		if($facture=='all') {
			$where = array('t_date_facture>='=>$from,'t_date_facture<='=>$to);

		} else {

			$where = array('t_date_facture>='=>$from,'t_date_facture<='=>$to,'t_id'=>$facture);
		}
