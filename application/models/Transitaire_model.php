...
/* BEGIN SHOW RISTOURNE FARWARDING AGENT*/
	
	public function get_ristourne($tra_id) { 
		$validation="valide";
			$where = "t_validation='$validation' AND t_transitaire=".$tra_id;
		
		$ristourne = $this->db->select('*')->from('trips')->where($where)->order_by('t_id','desc')->get()->result_array();
		if(!empty($ristourne)) {
			foreach ($ristourne as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
				$newtripdata[$key]['t_customer_details'] =  $this->db->select('*')->from('customers')->where('c_id',$tripdataval['t_customer_id'])->get()->row();
				//$newtripdata[$key]['t_driver_details'] =   $this->db->select('*')->from('drivers')->where('d_id',$tripdataval['t_driver'])->get()->row();
			}
...
/*END SHOW RISTOURNE FORWARDING AGENT*/		
...
//		return $this->db->select('*')->from('trip_compteur')->get()->result_array();
	...
	public function get_facture_transitaire($tra_id) { 
		
			$where = "t_transitaire=$tra_id";
	//	$conteneur =  $this->db->select('*')->from('trips,trip_conteneur')->where($where)->get()->result_array();	
