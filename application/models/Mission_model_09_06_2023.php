...
/*BEGIN AJAX*/	
 ...
 { //AND t_id NOT IN ( SELECT pr_trip_id FROM trip_pregate )";
 // $this->db->where('co_trip_id', $mi_trip_id);
$where="";
	echo $where=" pr_trip_id NOT IN (SELECT mi_trip_id FROM trip_mission )";	//exit; 
//	 $where="pr_id NOT IN (SELECT pr_trip_id FROM trip_mission,trip_pregate  WHERE pr_trip_id=mi_trip_id)";	 
 //	 $where="SELECT * FROM trip_pregate pr_id NOT IN (SELECT mi_trip_id FROM trip_mission )";	 
/* $this->db->where($where);
  $query = $this->db->order_by("pr_date_reception", "ASC");
  $query = $this->db->get("trip_pregate");*/
 
/*$query = $this->db->select('*')->from('trip_pregate')->where($where)->order_by('pr_id','desc')->group_by('pr_id')->get()->row();	 
 return $query->result();*/
	 
/* $this->db->where($where);	 
  $this->db->order_by('pr_id','desc');
  $query = $this->db->get("trip_pregate");//exit;
  return $query->result();*/
