...
return $this->db->insert('piece_rechange_pneu_compteur', $dataCompteur_bon_carburant);		
     ...
     public function get_numero_pneu_entree() { 
			return $this->db->count_all_results('piece_rechange_pneu_compteur_entree');
}
	
     public function get_numero_pneu_sortie() { 
			return $this->db->count_all_results('piece_rechange_pneu_compteur_sortie');
}	
...
/*	public function getall_entree_dans_sortie_list() { 
		echo "ddd";exit;
$type='ENTREE';		
		return $this->db->select('*')->from('piece_rechange_pneu')->where('pr_type',$type)->order_by('pr_id','desc')->get()->result_array();	exit;	
	}*/	
	
	public function get_entree_dans_sortie_list() { 
		return $this->db->select('*')->from('piece_rechange_pneu')->get()->result_array();
	}		
	...
	public function getall_piece_rechange_pneu_reference_list() { 
		return $this->db->select('*')->from('piece_rechange_pneu')->get()->result_array();
	}	
...
/*DEBUT LISTE PNEU*/	
  public function getall_piece_rechange_pneu_entree() { 
		
$type='ENTREE';		
		$piece_rechange_pneu = $this->db->select('*')->from('piece_rechange_pneu')->where('pr_type',$type)->order_by('pr_id','desc')->get()->result_array();
		if(!empty($piece_rechange_pneu)) {//exit;
			foreach ($piece_rechange_pneu as $key => $piece_rechange_pneus) {
				$newpiece_rechange_pneu[$key] = $piece_rechange_pneus;
				$newpiece_rechange_pneu[$key]['fournisseur'] =  $this->db->select('f_numero,f_name')->from('piece_rechange_fournisseurs')->where('f_id',$piece_rechange_pneus['pr_fournisseur_id'])->get()->row();
				$newpiece_rechange_pneu[$key]['marque'] =  $this->db->select('m_name')->from('piece_rechange_marque')->where('m_id',$piece_rechange_pneus['pr_marque_id'])->get()->row();
...
/*FIN LISTE PNEU ENTRE*/	
	

/*DEBUT LISTE PNEU SORTIE*/	
  public function getall_piece_rechange_pneu_sortie() { 
...
$type='SORTIE';		
		...
		if(!empty($piece_rechange_pneu)) {//exit;
			foreach ($piece_rechange_pneu as $key => $piece_rechange_pneus) {
				...
				$newpiece_rechange_pneu[$key]['vehicule'] =  $this->db->select('v_registration_no')->from('vehicles')->where('v_id',$piece_rechange_pneus['pr_vehicule_id'])->get()->row();
				...
				$newpiece_rechange_pneu[$key]['position'] =  $this->db->select('p_name')->from('piece_rechange_position')->where('p_id',$piece_rechange_pneus['pr_position_id'])->get()->row();			
				
				$newpiece_rechange_pneu[$key]['numero_be'] =  $this->db->select('pr_numero')->from('piece_rechange_pneu')->where('pr_id',$piece_rechange_pneus['pr_numero_bon_entree_id'])->get()->row();					
...
/*FIN LISTE PNEU SORTIE*/		
    ...
    public function getall_piece_rechange_fournisseurs_entree() { 
		
		$requete="";
		$type='PNEU';
		$requete="f_type=$type";
		//'f_type',$type
//		return $this->db->select('*')->from('piece_rechange_fournisseurs')->where($requete)->get()->result_array();		
		return $this->db->select('*')->from('piece_rechange_fournisseurs')->get()->result_array();		
