...
class eir_model extends CI_Model{
 ...
 public function getall_eir_plein_reports($from,$to) { 
	   
	

		$where = "";
		$where = "t_id=ei_pregate_id AND ei_date BETWEEN '$from' AND '$to'";//exit;
		


		  return $this->db->select('*')->from('trip_mission_eir_plein,trip_pregate,trips,trip_mission')->where($where)->order_by('ei_id','desc')->group_by('ei_id')->get()->result_array();		 
	}
...
/*     public function get_numeroeir_plein() { 
 ...
 /* Fin carte péage*/    
    ...
    public function getall_eir_plein() { 
		...
		/*$where = "";
		$where = "t_id=ei_pregate_id AND ei_conteneur_id=co_id";//exit;

		  return $this->db->select('*')->from('trip_mission_eir_plein,trip_pregate,trips,trip_mission,trip_conteneur')->where($where)->order_by('ei_id','desc')->group_by('ei_id')->get()->result_array();*/		
		$neweir_plein = array();

     $eir_plein = $this->db->select('*')->from('trip_mission_eir_plein')->order_by('ei_id','desc')->get()->result_array();
		if(!empty($eir_plein)) {
			foreach ($eir_plein as $key => $tripdataval) {
				$neweir_plein[$key] = $tripdataval;
				$neweir_plein[$key]['pregate'] =  $this->db->select('*')->from('trip_pregate')->where('pr_trip_id',$tripdataval['ei_pregate_id'])->get()->row();
				$neweir_plein[$key]['conteneur'] =  $this->db->select('co_numero_conteneur,co_type_conteneur')->from('trip_conteneur')->where('co_id',$tripdataval['ei_conteneur_id'])->get()->row();
				$neweir_plein[$key]['facture'] =   $this->db->select('*')->from('trips')->where('t_id',$tripdataval['ei_pregate_id'])->get()->row();

				$neweir_plein[$key]['mission'] =   $this->db->select('*')->from('trip_mission')->where('mi_trip_id',$tripdataval['ei_pregate_id'])->get()->row();
	...
	public function editeir_plein($ei_id) { 
		return $this->db->select('*')->from('trip_mission_eir_plein')->where('ei_id',$ei_id)->get()->result_array();
	}
...
/* Debut ajout*/
				...
				$neweir_plein[$key]['conteneur'] =   $this->db->select('*')->from('trip_conteneur')->where('co_id',$tripdataval['ei_conteneur_id'])->get()->row();				
...
//  $output .= '<option value="'.$row->co_id.'">'.$row->co_numero_conteneur.'</option>';
 ...
 function fetch_information_mission($mi_lieu_restitution_id)
 { 
...
AND drivers.d_id=trip_mission.mi_driver_id
AND vehicle_remorque.vr_id=trip_mission.mi_vehicle_remorque_id
AND trip_mission.mi_id=$mi_lieu_restitution_id";
...
/* function fetch_conteneur($mi_trip_id)
 { 
  ...
  $requete = '';
  $requete = "pr_trip_id=co_trip_id AND pr_trip_id=$mi_trip_id";
  $this->db->where($requete);
  $this->db->order_by('co_id', 'ASC');
  $query = $this->db->get('trip_pregate,trip_conteneur');
  $output = '<option value="">Selectionnez le conteneur</option>';
  foreach($query->result() as $row)
  {
   $output .= '<option value="'.$row->co_id.'">'.$row->co_numero_conteneur.'</option>';
 ...
 function fetch_lieu_restitution1($mi_trip_id)
 { 
  
  $where = ""; 	 
 /* $where = "";	*/ 
	 $where = "pr_trip_id=mi_trip_id AND pr_id=$mi_trip_id";
  $this->db->where($where);
//  $this->db->where('t_id', $mi_trip_id,'t_compagnie','clr_compagnie_id');
  $this->db->order_by('mi_id', 'ASC');
  $query = $this->db->get('trip_mission,trip_pregate');
  $output = '<option value="">Selectionnez la mission bn </option>';
  foreach($query->result() as $row)
  {
   $output .= '<option value="'.$row->mi_id.'">'.$row->mi_numero.'</option>';
  }
 ...
 function fetch_lieu_restitution($ei_trip_id)
 { 
  
  $where = ""; 	 
 /* $where = "";	*/ 
	 $where = "t_id=$ei_trip_id AND t_compagnie=cc_id AND cc_id=clr_compagnie_id";
  $this->db->where($where);
//  $this->db->where('t_id', $mi_trip_id,'t_compagnie','clr_compagnie_id');
  ...
  return $output;
	 ...
	 function fetch_conteneur_vide($ei_trip_id)
 { 
  $this->db->where('co_trip_id', $ei_trip_id);
  $this->db->order_by('co_id', 'ASC');
  $query = $this->db->get('trip_conteneur');
...
/*BEGIN EIR VIDE*/	
 ...
 public function getall_eir_vide_reports($from,$to) { 
		...
		$where = "t_id=ei_pregate_id AND ei_date BETWEEN '$from' AND '$to'";//exit;
		  ...
		  return $this->db->select('*')->from('trip_mission_eir_vide,trip_pregate,trips,trip_mission')->where($where)->order_by('ei_id','desc')->group_by('ei_id')->get()->result_array();		 
	}	
	...
	*/
	...
	/*	$where = "";
		$where = "t_id=ei_pregate_id AND ei_conteneur_id=co_id";//exit;
		  ...
		  return $this->db->select('*')->from('trip_mission_eir_vide,trip_pregate,trips,trip_mission,trip_conteneur')->where($where)->order_by('ei_id','desc')->group_by('ei_id')->get()->result_array();*/	
		$neweir_vide = array();

     $eir_vide = $this->db->select('*')->from('trip_mission_eir_vide')->order_by('ei_id','desc')->get()->result_array();
		if(!empty($eir_vide)) {
			foreach ($eir_vide as $key => $tripdataval) {
				$neweir_vide[$key] = $tripdataval;
				$neweir_vide[$key]['pregate'] =  $this->db->select('*')->from('trip_pregate')->where('pr_trip_id',$tripdataval['ei_pregate_id'])->get()->row();
				$neweir_vide[$key]['conteneur'] =  $this->db->select('co_numero_conteneur,co_type_conteneur')->from('trip_conteneur')->where('co_id',$tripdataval['ei_conteneur_id'])->get()->row();
				$neweir_vide[$key]['facture'] =   $this->db->select('*')->from('trips')->where('t_id',$tripdataval['ei_pregate_id'])->get()->row();

				$neweir_vide[$key]['mission'] =   $this->db->select('*')->from('trip_mission')->where('mi_trip_id',$tripdataval['ei_pregate_id'])->get()->row();
	...
	public function editeir_vide($ei_id) { 
