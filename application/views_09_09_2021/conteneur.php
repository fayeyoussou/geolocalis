                    ...
                    <table id="conteneurtbl" class="table table-bordered table-striped">
                      ...
                      <?php if(!empty($conteneur)){  $count=1;
                        ...
                        if($conteneurs['co_type_conteneur']!='') {
                          if($conteneurs['co_type_conteneur']=='20_pieds') { $type_conteneur = $conteneurs['zone_name']->z_montant_conteneur_20; $type='20 pieds'; } else { $type_conteneur = $conteneurs['zone_name']->z_montant_conteneur_40;$type='40 pieds'; }}  ?>
                          <?php echo output($type);?></td>           
                           <td> <?php if(isset($conteneurs['nature_name']->na_name)){echo output($conteneurs['nature_name']->na_name);}?></td>
                           <td> <?php echo output($montant=$type_conteneur+$conteneurs['nature_name']->na_montant);?></td>
