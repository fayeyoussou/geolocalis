...
if($ie_espece_cheque=="CHEQUE"){	
  ...
  //      $ie_banque_emettrice_id = $numero_facture['ie_banque_emettrice_id'];
        $ie_banque_receptrice_id = $numero_facture['ie_banque_receptrice_id'];  	
 $soldeBase =0;
			$soldeBase = $this->db->select('*')->from('incomeexpense')->order_by('ie_id desc')->limit(1)->get()->row()->ie_solde;	//exit;
$solde= $soldeBase - $ie_amount;	
	$addincomexpense = array('ie_type_mouvement'=>$ie_type_mouvement,'ie_date'=>$ie_date,'ie_numero_enregistrement'=>$ie_numero_enregistrement,'ie_amount'=>$ie_amount,'ie_numero'=>$ie_numero,'ie_description'=>$ie_description,'ie_objet'=>$ie_objet,'ie_type'=>$ie_type,'ie_banque_receptrice_id'=>$ie_banque_receptrice_id,'ie_espece_cheque'=>$ie_espece_cheque,'ie_transitaire_id'=>$ie_transitaire_id,'ie_amount_ristourne'=>$ie_amount_ristourne,'ie_solde'=>$solde);
}			
...
//		$data['driverlist'] = $this->trips_model->getall_driverlist();
	...
	/* DEBUT AFFICHAGE RISTOURNE*/
	
			public function addristourne()	{
		if(isset($_POST['incomeexpensereport'])) {
			$incomeexpensereport = $this->Incomexpense_model->incomexpense_reports($this->input->post('incomeexpense_from'),$this->input->post('incomeexpense_to'),$this->input->post('ie_ieb_id')); 
...
//		$data['vehiclelist'] = $this->vehicle_model->getall_vehicle();
		...
		$data['transitairelist'] = $this->Incomexpense_model->getall_transitairelist_ajax();       
        $data['customerlist'] = $this->Incomexpense_model->getall_customer();			
			
			if(isset($_POST['incomeexpensereport'])) {
				$this->template->template_render('incomexpense_add_ristourne',$data);
...
//		$data['vechiclelist'] = $this->trips_model->getall_vechicle();
	...
	public function editincomexpense_banque()
	{
		$this->load->model('trips_model');
		$data['vechiclelist'] = $this->trips_model->getall_vechicle();
		$data['reglementlist'] = $this->Incomexpense_model->getall_banque();//
...
// END UPDATE BILL PAYMENT		
	...
	public function updateincomexpense_banque()
	{
		$testxss = xssclean($_POST);
		if($testxss){
			$response = $this->Incomexpense_model->updateincomexpense($this->input->post());
...
//			$incomeexpensereport = $this->Incomexpense_model->incomexpense_reports($this->input->post('incomeexpense_from'),$this->input->post('incomeexpense_to'),$this->input->post('ie_ieb_id'),$this->input->post('ie_banque_emettrice_id'),$this->input->post('ie_banque_receptrice_id')); //echo $incomeexpensereport;
			...
			// permet d’afficher tous les elements de la liste		
		$data['depense_caisse'] = $this->Incomexpense_model->get_incomexpense_depense_caisse();	
				
		$data['alimentation_caisse'] = $this->Incomexpense_model->get_incomexpense_alimentation_caisse();	
				
		$data['entree'] = $this->Incomexpense_model->getall_facture_entree();	
				
		$data['sortie'] = $this->Incomexpense_model->getall_facture_sortie();	
		...
		$data['facturelist'] = $this->Incomexpense_model->getall_facture();
		$data['numero_compteurcaisse'] = $this->Incomexpense_model->get_numero_compteur_caisse();
		$data['solde'] = $this->Incomexpense_model->getsolde_caisse();		//$data['facturelist'] = $this->Incomexpense_model->getall_facture_validee(); //lister les factures validées
...
//		$data['vehiclelist'] = $this->vehicle_model->getall_vehicle();
	...
	public function insertincomexpense_caisse_depense()
	{
		$testxss = xssclean($_POST);
		if($testxss){
		...
		$ie_created_date = $banque['ie_created_date'];
        $ie_type = $banque['ie_type'];
		$ie_numero_enregistrement = $banque['ie_numero_enregistrement'];
		//$ie_objet = "BANQUE ".$ie_type;

		$ie_objet = "Dépenses journalieres";
...
$solde= $soldeBase - $ie_amount;			
		...
		/*if($ie_type=="expense"){*/
			$addincomexpense = array('ie_date'=>$ie_date,'ie_type_mouvement'=>$ie_type_mouvement,'ie_amount'=>$ie_amount,'ie_type'=>$ie_type,'ie_numero_enregistrement'=>$ie_numero_enregistrement,'ie_objet'=>$ie_objet,'ie_created_date'=>$ie_created_date,'ie_espece_cheque'=>$ie_espece_cheque,'ie_solde'=>$solde);		
	
			
			
$response = $this->Incomexpense_model->add_incomexpense($addincomexpense);
			$last_incomexpense_id = 0;	
			$last_incomexpense_id= $this->db->insert_id();			
...
/*FIN ADD CAISSE*/	
	...
	/*DEBUT ADD CAISSE ALIMENTATION CAISSE*/

	public function insertincomexpense_caisse_alim_caisse()
	{
		$testxss = xssclean($_POST);
		if($testxss){
...
$solde= $soldeBase + $ie_amount;			
			...
			$addincomexpense = array('ie_compte_id'=>$ie_compte_id,'ie_espece_cheque'=>$ie_espece_cheque,'ie_date'=>$ie_date,'ie_type_mouvement'=>$ie_type_mouvement,'ie_amount'=>$ie_amount,'ie_type'=>$ie_type,'ie_numero'=>$ie_numero,'ie_numero_enregistrement'=>$ie_numero_enregistrement,'ie_created_date'=>$ie_created_date,'ie_description'=>$ie_description,'ie_solde'=>$solde);			
$response = $this->Incomexpense_model->add_incomexpense($addincomexpense);
			$last_incomexpense_id = 0;	
			$last_incomexpense_id= $this->db->insert_id();			
...
/*DEBUT AJOUT JOURNAL CAISSE*/
//$this->db->select_sum('bc_restant');
/*$this->db->select('isc_solde');
$this->db->from('incomeexpense_solde_caisse');
//$this->db->where('bc_id', $id_bon_carburant);
$this->db->order_by('isc_id desc');
$this->db->limit(1);
$this->db->get();*/	 
	 
/*echo $soldeBase = $this->db->select('*')->from('incomeexpense_solde_caisse')->order_by('isc_id desc')->limit(1)->get()->row()->isc_solde;	//exit;
$solde= $soldeBase + $ie_amount;	 
        $isc_objet = "ALIMENTATION CAISSE";	
	$add_incomeexpense_solde_caisse = '';
		$add_incomeexpense_solde_caisse = array('isc_objet'=>$isc_objet,'isc_espece_cheque'=>$ie_espece_cheque,'isc_date'=>$ie_date,'isc_type_mouvement'=>$ie_type_mouvement,'isc_amount'=>$ie_amount,'isc_type'=>$ie_type,'isc_numero_enregistrement'=>$ie_numero_enregistrement,'isc_created_date'=>$ie_created_date,'isc_solde'=>$solde,'isc_description'=>$ie_description);		
		$response_add_journal = $this->db->insert('incomeexpense_solde_caisse',$add_incomeexpense_solde_caisse);*/
	 
	 
/*FIN AJOUT JOURNAL CAISSE*/	 
...
/*FIN ADD CAISSE ALIMENTATION CAISSE*/	
...
/*DEBUT DETAIL DEPENSE CAISSE SUITE*/	
	public function details_caisse_suite() //function that displays details
	{
		$data = array();
		$ie_id = $this->uri->segment(3);
		$ioc_id = $this->uri->segment(4);

		$incomexpensedetails = $this->Incomexpense_model->get_incomexpensedetails_caisse_suite($ioc_id);
		...
		if(isset($incomexpensedetails[0]['ie_id'])) {
           ...
           $data['comptelist'] = $this->Incomexpense_model->getall_compte();
			//exit;
			$data['depense_caisselist'] = $this->Incomexpense_model->get_depensecaisse_suitedetails($ioc_id);			
		...
		$this->template->template_render('incomexpense_details_caisse_suite',$data);

        
    }	
/*FIN DETAIL DEPENSE CAISSE SUITE*/	
	
	
	
/*DEBUT DETAIL DEPENSE CAISSE*/	
	public function details_caisse() //function that displays details
	{
		$data = array();
		$ie_id = $this->uri->segment(3);

		$incomexpensedetails = $this->Incomexpense_model->get_incomexpensedetails_caisse($ie_id);
		...
		if(isset($incomexpensedetails[0]['ie_id'])) {
           ...
           $data['comptelist'] = $this->Incomexpense_model->getall_compte();
			...
			$data['depense_caisselist'] = $this->Incomexpense_model->get_depensecaissedetails($incomexpensedetails[0]['ie_id']);			
		...
		$this->template->template_render('incomexpense_details_caisse',$data);

        
    }	
/*FIN DETAIL DEPENSE CAISSE*/
...
public function incomexpense_generation_pdf()
	{
		...
		if(isset($incomexpensedetails[0]['ie_id'])) {
           ...
           $data['facturelist'] = $this->Facture_model->getall_facture();
			...
			$data['banquedetails'] = $this->Incomexpense_model->get_banquedetails($incomexpensedetails[0]['ie_id']);			
			$data['station_service'] = $this->Incomexpense_model->get_banquedetails($banquedetails[0]['ie_id']);			
...
/*DEBUT INSERTION DEPENSES CAISSE*/	
	public function insert_depensecaisse()
	{
		$depensecaisse = $this->input->post();
		//exit;
		
		//$ioc_date = $depensecaisse['ioc_date'];
		$ioc_ie_id = $depensecaisse['ioc_ie_id'];
		$ioc_amount = $depensecaisse['ioc_amount'];
        $ioc_compte_id = $depensecaisse['ioc_compte_id'];
		$ioc_designation = $depensecaisse['ioc_designation'];
        $ioc_type = $depensecaisse['ioc_type'];        
		$ioc_created_date = $depensecaisse['ioc_created_date'];
		$ioc_numero_caisse = $depensecaisse['ioc_numero_caisse'];
			
	 $addincomeexpense_operation_caisse = array('ioc_numero_caisse'=>$ioc_numero_caisse,'ioc_ie_id'=>$ioc_ie_id,'ioc_amount'=>$ioc_amount,'ioc_compte_id'=>$ioc_compte_id,'ioc_designation'=>$ioc_designation,'ioc_type'=>$ioc_type,'ioc_created_date'=>$ioc_created_date);
		

			$this->db->insert('incomeexpense_operation_caisse',$addincomeexpense_operation_caisse);		
		...
		if($this->db->insert_id()) {
			...
			redirect('incomexpense/details_caisse/'.$ioc_ie_id);
			//echo "1";
		} else {
			$this->session->set_flashdata('warningmessage', 'Error!. Please try again');
			redirect('incomexpense/details_caisse/'.$ioc_ie_id);//echo "2";exit;
		}
	}	
/*FIN INSERTION DEPENSES CAISSE*/
	
	
/*DEBUT INSERTION DEPENSES CAISSE SUITE*/	
	public function insert_depensecaisse_suite()
	{
		$depensecaisse = $this->input->post();
		//exit;
		...
		$ioc_created_date = $depensecaisse['ioc_created_date'];
		//$ioc_numero_caisse = $depensecaisse['ioc_numero_caisse'];
		$ioc_caisse_id = $depensecaisse['ioc_caisse_id'];
		
	 $addincomeexpense_operation_caisse = array('ioc_ie_id'=>$ioc_ie_id,'ioc_amount'=>$ioc_amount,'ioc_compte_id'=>$ioc_compte_id,'ioc_designation'=>$ioc_designation,'ioc_type'=>$ioc_type,'ioc_created_date'=>$ioc_created_date,'ioc_caisse_id'=>$ioc_caisse_id);
		...
		if($this->db->insert_id()) {
			...
			redirect('incomexpense/details_caisse_suite/'.$ioc_ie_id.'/'.$ioc_caisse_id);
			//echo "1";
		} else {
			$this->session->set_flashdata('warningmessage', 'Error!. Please try again');
			redirect('incomexpense/details_caisse_suite/'.$ioc_ie_id.'/'.$ioc_caisse_id);//echo "2";exit;
		}
	}	
/*FIN INSERTION DEPENSES CAISSE SUITE*/	
...
// BEGIN delete paiement	
	public function depense_caisse_suite_delete()
	{
		$ioc_id = $this->uri->segment(5);
		$response = $this->db->delete('incomeexpense_operation_caisse', array('ioc_id' =>$ioc_id));
		if($response) {
			$this->session->set_flashdata('successmessage', 'Payment record deleted successfully..');
		} else {
			$this->session->set_flashdata('warningmessage', 'Unexpected error..Try again');
		}
		redirect('incomexpense/details_caisse_suite/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
	}
	
/*DEBUT DELETE SUITE*/	
// BEGIN delete paiement	
	public function depense_caisse_delete()
	{
		$ioc_id = $this->uri->segment(3);
		$response = $this->db->delete('incomeexpense_operation_caisse', array('ioc_id' =>$ioc_id));
		if($response) {
			$this->session->set_flashdata('successmessage', 'Payment record deleted successfully..');
		} else {
		...
		redirect('incomexpense/details_caisse/'.$this->uri->segment(4));
	}	
/*FIN DELETE SUITE*/	
...
//		$data['conteneur'] = $this->conteneur_model->getall_conteneur();
 ...
 { //echo "bjr";exit;
  if($this->input->post('ie_transitaire_id'))
  {
   echo $this->incomexpense_model->fetch_bon_carburant($this->input->post('ie_transitaire_id'));
  }
 }	
	
/* BEGIN AJAX VEHICULE*/	
	 function fetch_ristourne()
 { echo "bjr";//exit;
  if($this->input->post('ie_transitaire_id'))
  {
   echo $this->incomexpense_model->fetch_ristourne($this->input->post('ie_transitaire_id'));
  }//else{echo "rien";}
 }	
	
	public function addincomexpense_piece_rechange()
	{
		$this->load->model('trips_model');
		$this->load->model('incomexpense_model');
		$data['vechiclelist'] = $this->trips_model->getall_vechicle();		$data['categorielist'] = $this->incomexpense_model->getall_categorie_piece_rechange();
		$data['incomexpense'] = $this->Incomexpense_model->getall_incomexpense_piece_rechange();
		
		$this->template->template_render('incomexpense_piece_rechange_add',$data);
	}
	
	public function insertincomexpense_piece_rechange()
	{
		$testxss = xssclean($_POST);
		if($testxss){
			$response = $this->Incomexpense_model->add_incomexpense_piece_rechange($this->input->post());
			if($response) {
				$this->session->set_flashdata('successmessage', 'New '.$this->input->post('ie_type').' added successfully..');
			} else {
				$this->session->set_flashdata('warningmessage', 'Something went wrong..Try again');
			}
			redirect('incomexpense/addincomexpense_piece_rechange');
		} else {
			$this->session->set_flashdata('warningmessage', 'Error! Your input are not allowed.Please try again');
			redirect('incomexpense/addincomexpense_piece_rechange');
		}
...
/*DEBUT RISTOURNE*/
 function fetch_facture_transitaire()
 { //echo "bjr";//exit;
  if($this->input->post('mi_trip_id'))
  {
   echo $this->Incomexpense_model->fetch_facture_transitaire($this->input->post('mi_trip_id'));
  }
 }	
	

	
	public function insertincomexpense_banque_sortie_ristourne()
	{
		/*$testxss = xssclean($_POST);
		if($testxss){
			*/
...
/*				if($ie_compte_id==140){
		...
		$addincomexpense = array('ie_date'=>$ie_date,'ie_banque_emettrice_id'=>$ie_banque_emettrice_id,'ie_type_mouvement'=>$ie_type_mouvement,'ie_amount'=>$ie_amount,'ie_description'=>$ie_description,'ie_type'=>$ie_type,'ie_numero'=>$ie_numero,'ie_numero_enregistrement'=>$ie_numero_enregistrement,'ie_transitaire_id'=>$ie_transitaire_id,'ie_trip_id'=> implode(',', (array) $this->input->post('ie_trip_id')));
//	}
...
if($response) {
			...
			redirect('incomexpense/addristourne');
//				redirect('incomexpense/addincomexpense');
/*	} else {
			$this->session->set_flashdata('warningmessage', 'Error! Your input are not allowed.Please try again');
			redirect('incomexpense/addristourne');
