...
/*END FILTRE*/		
		...
		$data['pregate_instancelist'] = $this->pregate_model->getall_pregateinstance();
...
/*BEGIN GESTION TRANSPORT*/
		public function pregate_gestion_transport()
	{
		$this->load->model('trips_model');
		
/*BEGIN FILTRE*/	
/*		if(isset($_POST['pregate_add_report'])) {
			$pregate_report = $this->pregate_model->pregate_reports_transport($this->input->post('from'),$this->input->post('to'),$this->input->post('pr_statut'),$this->input->post('type'),$this->input->post('compagnie'),$this->input->post('pied')); //echo 
				if(empty($pregate_report)) {
					$this->session->set_flashdata('warningmessage', 'No data found..');
					$data['pregate'] = '';
				} else {
		...
		if(isset($_POST['incomeexpensereport'])) {
			$data['pregate'] = $this->pregate_model->getall_pregate_transport_reports($this->input->post('incomeexpense_from'),$this->input->post('incomeexpense_to'));
			/*if(empty($incomeexpensereport)) {
				$this->session->set_flashdata('warningmessage', 'No data found..');
				$data['pregate'] = '';
			} else {
				unset($_SESSION['warningmessage']);
				$data['pregate'] = $incomeexpensereport;
			}*/
		}			
			else{
				$data['pregate'] = $this->pregate_model->getall_pregate_transport();
			}		
/*END FILTRE*/		
		...
		$this->template->template_render('pregate_gestion_transport',$data);
	}
/*END GESTION TRANSPORT*/	
...
/* BEGIN ADD PREGATE */	
	...
	public function insertpregate()
	{
		...
		$facture = $this->input->post('pr_trip_id');
...
/* MODIFICATION*/
		...
		$this->db->where('t_id',$this->input->post('pr_trip_id'));
...
//		$data['vechiclelist'] = $this->trips_model->getall_vechicle();
		$data['facturelist'] = $this->pregate_model->getall_facture_edit();
		$data['conteneurlist'] = $this->pregate_model->getall_conteneur();
...
public function pregate_generation_pdf()
	{
		$data = array();
		$pr_id = $this->uri->segment(3);
		$pregatedetails = $this->pregate_model->get_pregatedetails_pdf($pr_id);				if(isset($pregatedetails[0]['pr_id'])) {
            ...
            $data['compagniedetails'] = $this->compagnie_model->get_compagniedetails($pregatedetails[0]['t_compagnie']);            
			$customerdetails = $this->customer_model->get_customerdetails($pregatedetails[0]['t_customer_id']);
			$data['customerdetails'] = (isset($customerdetails[0]['c_id']))?$customerdetails[0]:'';
			
			$data['conteneurdetails'] = $this->facture_model->get_conteneurdetails($pregatedetails[0]['t_id']);
			

			$data['missiondetails'] = $this->pregate_model->get_mission($pregatedetails[0]['t_customer_id']);
