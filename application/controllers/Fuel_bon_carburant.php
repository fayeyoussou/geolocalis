...
/*END PAIEMENT*/	
	...
	public function editfuel_bon_carburant()
	{
		...
		$data['fuel_compagnielist'] = $this->fuel_compagnie_model->getall_fuel_compagnie();		
		$this->template->template_render('fuel_bon_carburant_edit',$data);
	}
...
//Prix du litre du carburant	
			...
			$montant_carburant = $prix_carburant * $bc_quantite;//exit;		

			$maj_bon_carburant = array('bc_vehicule_id'=>$bc_vehicule_id,'bc_date'=>$bc_date,'bc_quantite'=>$bc_quantite,'bc_desc'=>$bc_desc,'bc_compagnie_id'=>$bc_compagnie_id,'bc_driver_id'=>$bc_driver_id,'bc_montant'=>$montant_carburant);		
		$this->db->where('bc_id',$this->input->post('bc_id'));	
		$response = $this->db->update('fuel_bon_carburant',$maj_bon_carburant);				
/*END*/			
			
			
			//$response = $this->fuel_bon_carburant_model->updatefuel_bon_carburant($maj_bon_carburant);
