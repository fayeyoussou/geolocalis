...
//		$data['vechiclelist'] = $this->invoice_model->getall_vechicle();
		$data['transitairelist'] = $this->invoice_model->getall_transitairelist();
...
//        if(isset($invoicedetails[0]['t_id'])) {
			$customerdetails = $this->customer_model->get_customerdetails($invoicedetails[0]['i_customer']);
            
			//$customerdetails = $this->customer_model->get_customerdetails($invoicedetails[0]['t_customer_id']);            
            
			$driverdetails = $this->drivers_model->get_driverdetails($invoicedetails[0]['i_driver']);
//			$driverdetails = $this->drivers_model->get_driverdetails($invoicedetails[0]['t_driver']);

            
            $data['paymentdetails'] = $this->invoice_model->get_paymentdetails($invoicedetails[0]['t_id']);
	...
	public function invoicepayment()
	{
		...
		if($this->db->insert_id()) {
			$addincome = array('ie_i_id'=>$this->input->post('tp_i_id'),'ie_date'=>date('Y-m-d'),'ie_type'=>'income','ie_description'=>'payment from invoice and '.$this->input->post('tp_notes'),'ie_amount'=>$this->input->post('tp_amount'),'ie_created_date'=>date('Y-m-d'));
