...
class Fuel_carte_peage_recharge extends CI_Controller {

	 function __construct()
     {
          parent::__construct();
          $this->load->database();
          $this->load->model('fuel_carte_peage_recharge_model');
	...
	{
		$data['fuel'] = $this->fuel_carte_peage_recharge_model->getall_fuel();
		$this->template->template_render('fuel_carte_peage_recharge',$data);
	}
	public function addfuel_carte_peage_recharge()
	{
		//$this->load->model('trips_model');
		$this->load->model('fuel_carte_peage_model');
//		$data['driverlist'] = $this->trips_model->getall_driverlist();
		$data['carte_peagelist'] = $this->fuel_carte_peage_model->getall_fuel_carte_peage();
		$this->template->template_render('fuel_carte_peage_recharge_add',$data);
	}
	public function insertfuel()
	{
		$testxss = xssclean($_POST);
		if($testxss){
			$response = $this->fuel_carte_peage_recharge_model->add_fuel($this->input->post());
			if($response) {
...
/*				$is_include = $this->input->post('exp');
			...
			redirect('fuel_carte_peage_recharge');
		} else {
...
//		$data['driverlist'] = $this->trips_model->getall_driverlist();
		...
		$data['fueldetails'] = $this->fuel_carte_peage_recharge_model->editfuel($ccr_id);
		$this->template->template_render('fuel_carte_peage_recharge_add',$data);
	}
	...
	public function updatefuel()
	{
		$testxss = xssclean($_POST);
		if($testxss){
			$response = $this->fuel_carte_peage_recharge_model->updatefuel($this->input->post());
			if($response) {
	...
	{
		$ccr_id = $this->uri->segment(3);
		$response = $this->db->delete('fuel_carte_peage_recharge', array('ccr_id' =>$ccr_id));
		if($response) {
