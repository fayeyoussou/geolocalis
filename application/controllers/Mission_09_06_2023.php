...
/*		$testxss = xssclean($_POST);
	...
	$addincomexpense = array('mi_vehicle_id'=>$mi_vehicle_id,'mi_motif_perte_id'=>$mi_motif_perte_id,'mi_quantite_carburant'=>$mi_quantite_carburant,'mi_trip_id'=>$mi_trip_id,'mi_conteneur_id'=>$mi_conteneur_id,'mi_lieu_restitution_id'=>$mi_lieu_restitution_id,'mi_date_mission'=>$mi_date_mission,'mi_driver_id'=>$mi_driver_id,'mi_vehicle_remorque_id'=> $mi_vehicle_remorque_id,'mi_carte_peage_id'=>$mi_carte_peage_id,'mi_carte_peage_amount'=> $mi_carte_peage_amount,'mi_frais_ags'=>$mi_frais_ags,'mi_autre_frais'=> $mi_autre_frais,'mi_description'=> $mi_description,'mi_created_date'=> $mi_created_date,'mi_numero'=> $mi_numero); /**/  
...
//		$quantite_restante = $this->db->select('*')->from('fuel_bon_carburant')->where('bc_vehicule_id', $bc_vehicule_id)->order_by('bc_id','desc')->get()->row()->bc_restant;			
		...
		$quantite_restante_vehicule =0;
			$quantite_restante_vehicule = $carburant_restant-$mi_quantite_carburant;		 
	$maj_bon_carburant = array('bc_restant'=>$quantite_restante_vehicule);		
			
		//$response = $this->db->insert('fuel_bon_carburant',$maj_bon_carburant);	 

		$this->db->where('bc_id',$bc_id);
		$maj = $this->db->update('fuel_bon_carburant',$maj_bon_carburant);
