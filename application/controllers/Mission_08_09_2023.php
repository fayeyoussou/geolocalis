...
//$requete="";	 
 ...
 // $this->db->order_by('mi_id', 'desc');
  ...
  $row_mission = $query->row();
		...
		echo "type=".$mi_type_mission = $row_mission->mi_type_mission;		 
		...
		echo $mi_prime = $row_mission->mi_prime;
...
/* 
$profil_chauffeur = $this->db->select('*')->from('drivers')->where($requete_profil_chauffeur)->get()->row()->d_profil;		
exit;*/		
  //$this->db->where($requete_quantite_carb_vehicule);
//  $this->db->order_by('bc_id', 'desc');		
		
/*echo "profil".$profil_chauffeur = $this->db->select('*')->from('drivers')->where($requete_profil_chauffeur)->get()->row()->d_profil;*/		
...
/*BEGIN REQUETE*/
	...
	//echo "mi_driver_id=".$mi_driver_id;	
/**/ $this->db->where($requete_profil_chauffeur);
 // $this->db->order_by('mi_id', 'desc');
  ...
  $row_profil_drivers = $query_profil_drivers->row();
		...
		echo "ss".$profil_chauffeur = $row_profil_drivers->d_profil;//exit;
		}
...
$restant=0;	 
//$requete="";	 
	 $requete = "d_profil=$mi_driver_id";

  $this->db->where($requete);
  $query = $this->db->get('drivers');
$row_quantite = $query->row();
	 $carburant_restant = 0;
	 $carburant_quantite_totale = 0;echo "tester=";
	 if($row_quantite != ''){
		echo "dddd=".$carburant_restant = $row_quantite->d_profil;	exit;	 
...
{		
	if(($mi_type_mission == "LIVRAISON") || ($mi_type_mission == "RESTITUTION_VIDE") || ($mi_type_mission == "MISE_A_TERRE") || ($mi_type_mission == "DECROCHER"))
	{		
	//exit;		
	if(($mi_type_mission == "LIVRAISON") )
	{

		echo "s_driver_prime_permanent_eleve".$mp_amount = $s_driver_prime_permanent_eleve;	//exit;
	}

	if(($mi_type_mission == "RESTITUTION_VIDE") || ($mi_type_mission == "MISE_A_TERRE") || ($mi_type_mission == "DECROCHER") )
	{
		echo "s_driver_prime_permanent_bas". $mp_amount = $s_driver_prime_permanent_bas;	//exit;	
	}
}
}else{
		echo "s_driver_prime_permanent_prest". $mp_amount = $s_driver_prime_prestataire;	//exit;	
...
/*VERIFICATION STATUT CONDUCTEUR*/
	 ...
	 $requete_profil_chauffeur = "d_profil=$mi_driver_id";
...
/*$profil_chauffeur = $this->db->select('*')->from('drivers')->where($requete_profil_chauffeur)->get()->row()->d_profil;		
 ...
 // $this->db->order_by('mi_id', 'desc');
  ...
  $row_profil_drivers = $query_profil_drivers->row();
		...
		$profil_chauffeur = $row_profil_drivers->d_profil;
		}
		
		
		echo $s_driver_prime_permanent_eleve = $this->db->select('*')->from('settings')->get()->row()->s_driver_prime_permanent_eleve;
		echo $s_driver_prime_permanent_bas = $this->db->select('*')->from('settings')->get()->row()->s_driver_prime_permanent_bas;
echo $s_driver_prime_prestataire = $this->db->select('*')->from('settings')->get()->row()->s_driver_prime_prestataire;		
		exit;
/*DEBUT PRIME*/ 
//echo "ddd";//exit;
...
/*BEGIN PAIEMENT PRIME*/	
	...
	public function insert_paiementprime()
	{
		...
		$mpp_date = $paiementprime['mpp_date'];
		$mpp_mission_prime_id = $paiementprime['mpp_mission_prime_id'];
			
	$addincomexpense = array('mpp_numero'=>$mpp_numero,'mpp_date'=>$mpp_date,'mpp_mission_prime_id'=> implode(',', (array) $this->input->post('mpp_mission_prime_id'))); //exit; /**/  
