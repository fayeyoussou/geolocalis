...
class trip extends CI_Controller {
	...
	{
		$data['triplist'] = $this->trips_model->getall_trip();
...
/* debut ajout*/        
        $data['zonelist'] = $this->trips_model->getall_zonelist();
        $data['transitairelist'] = $this->trips_model->getall_transitairelist();
        $data['carte_peagelist'] = $this->trips_model->getall_carte_peagelist();
        $data['carte_carburantlist'] = $this->trips_model->getall_carte_carburantlist();
/* debut ajout*/        
	...
	public function inserttrip() 
	{
		...
		if($testxss){
			$response = $this->trips_model->add_trip($this->input->post());
...
/* debut ajout*/        
	...
	public function updatetrip()
	{
		$testxss = xssclean($_POST);
		if($testxss){
			$response = $this->trips_model->update_trip($this->input->post());
			if($response) {
				$this->session->set_flashdata('successmessage', 'New trip added successfully..');
			} else {
				$this->session->set_flashdata('warningmessage', 'Unexpected error..Try again');
...
//			$data['driverdetails'] =  (isset($driverdetails[0]['d_id']))?$driverdetails[0]:'';
	...
	public function trippayment()
	{
		...
		if($this->db->insert_id()) {
			...
			redirect('trip/details/'.$pyment['tp_trip_id']);
		} else {
	...
	public function trippayment_delete()
	{
		...
		redirect('trip/details/'.$this->uri->segment(4));
	}
	...
	public function sendtracking() {
		...
		if(!empty($gettemplate)) {
			...
			redirect('trip/details/'.$_GET['t_id']);
		}
	}
}
