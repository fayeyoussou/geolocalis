...
/*		if(isset($_POST['journal_bon_carburant'])) {
		...
		if(isset($_POST['journal_bon_carburant'])) {
			$data['fuel_bon_carburant'] = $this->fuel_bon_carburant_model->getall_fuel_bon_carburant_reports($this->input->post('from'),$this->input->post('to'),$this->input->post('vechicle'));//exit;
...
//		$quantite_restante = $this->db->select('*')->from('fuel_bon_carburant')->where('bc_vehicule_id', $bc_vehicule_id)->order_by('bc_id','desc')->get()->row()->bc_restant;			
	...
	$add_bon_carburant = array('bc_numero'=>$bc_numero,'bc_vehicule_id'=>$bc_vehicule_id,'bc_date'=>$bc_date,'bc_quantite'=>$bc_quantite,'bc_desc'=>$bc_desc,'bc_created_date'=> $bc_created_date,'bc_restant'=>$quantite_restante_vehicule,'bc_carte_id'=>$bc_carte_id);		
...
/* BEGIN */			
//	    $id_bon_carburant = $this->db->insert_id(); //exit;



//	$bc_vehicule_id = $this->db->select('*')->from('fuel_bon_carburant')->where('bc_id', $id_bon_carburant)->get()->row()->bc_vehicule_id;			
			
//echo "restant=". $bc_restant = $this->db->select('*')->from('fuel_bon_carburant')->where('bc_id', $id_bon_carburant)->get()->row()->bc_restant;
//$bc_restant = 0;
/*echo "restant REQUETE=". */
			
//$bc_restant = $this->db->select_sum('bc_restant')->from('fuel_bon_carburant')->where('bc_carte_id', $bc_carte_id)->get()->row()->bc_restant;
...
$data_restant = array('bc_restant' => $bc_qte_restant);	 
