...
//        $ie_banque_receptrice_id = $numero_facture['ie_banque_receptrice_id'];
	...
	$addincomexpense = array('ie_date'=>$ie_date,'ie_type_mouvement_caisse'=>$ie_type_mouvement_caisse,'ie_amount'=>$ie_amount,'ie_numero'=>$ie_numero,'ie_description'=>$ie_description,'ie_objet'=>$ie_objet,'ie_type'=>$ie_type,'ie_ieb_id'=>$ie_ieb_id);  
...
if($response) {
			...
			redirect('incomexpense/addincomexpense#banque');
//				redirect('incomexpense/addincomexpense');
	} else {
			$this->session->set_flashdata('warningmessage', 'Error! Your input are not allowed.Please try again');
			redirect('incomexpense/addincomexpense#banque');
