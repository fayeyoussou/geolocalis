...
/*		if(empty($conteneur)) {
		    ...
		    $data['detailconteneurs'] = $this->db->select('*')->from('trip_conteneur')->where('co_trip_id',$t_id)->order_by('co_id','desc')->get()->row();
...
public function conteneur_delete()
	{
		$st_id = $this->uri->segment(3);
		$returndata = $this->conteneur_model->conteneur_delete($st_id);
		if($returndata) {
